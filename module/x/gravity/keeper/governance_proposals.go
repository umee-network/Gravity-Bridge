...
func RegisterProposalTypes() {
	...
	if !gov1b1.IsValidProposalType(strings.TrimPrefix(metadata, prefix)) {
		gov1b1.RegisterProposalType(types.ProposalTypeIBCMetadata)
	}
	unhalt := "gravity/UnhaltBridge"
	if !gov1b1.IsValidProposalType(strings.TrimPrefix(unhalt, prefix)) {
		gov1b1.RegisterProposalType(types.ProposalTypeUnhaltBridge)
	}
	airdrop := "gravity/Airdrop"
	if !gov1b1.IsValidProposalType(strings.TrimPrefix(airdrop, prefix)) {
		gov1b1.RegisterProposalType(types.ProposalTypeAirdrop)
	}
}

func NewGravityProposalHandler(k Keeper) gov1b1.Handler {
